...
`r t` practice codes were discontinued in multiple years, meaning that they reported payments in one year, were missing in the following year's data, and then reappeared in a subsequent year's data before disappearing again.

The NHS Payments data also contains information on practice closures, under the column `Practice.Close.Date`.
...
### Pulse
We can also use data from [Pulse](https://www.pulsetoday.co.uk/news/practice-closures/) to identify practices that have closed. This data is stored in `pulse.csv`.
```{r Pulse}
...
ggplot(closure_IMD, aes(x = quintile)) +
  geom_bar()
```
We see that that closed practices tend to be in the higher quintiles of deprivation, with a mean IMD of `r mean(closure_IMD$IMD, na.rm = TRUE)`.

We then perform a Welch Two Sample t-test to determine if the mean IMD of closed practices is significantly different from the mean IMD of all practices.
```{r T-test}
